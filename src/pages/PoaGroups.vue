  ...
  methods: {
    ...
    columnChartSeries(array) {
      ...
      newArray.forEach(function(item) {
        ...
        for (let [key, value] of Object.entries(item)) {
          if (
            key !== "nid" &&
            key !== "field_proj_poagroup" &&
            key !== "field_poa_color" &&
            key !== "field_poagroup_abbreviation"
          ) {
            data.push(parseFloat(value));
          }
          if (key === "field_poagroup_abbreviation") {
            name = value;
          }
        ...
        financeringObj.color = item.field_poa_color;
    ...
    tableHeader(array) {
      ...
      Object.entries(singleFinancering).forEach(([key]) => {
        ...
        if (title === "field_proj_poagroup") {
          tableHeaders.push("Research group");
        }
        if (title !== "field_proj_poagroup" && title !== "nid") {
    ...
    async buildChart() {
      ...
      const response = await fetch(poaURL);
      if (response.ok) {
        ...
        var spline = {
          ...
          // data: [5, 7, 9, 6, 12, 4, 10, 8, 1, 3, 2, 6, 7, 11],
        };
